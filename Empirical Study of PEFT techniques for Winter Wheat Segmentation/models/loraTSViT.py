...
class LoraTSViT(nn.Module):
    ...
    """
    LoraTSViT: Same as TSViT, but the replace all nn.Linear by lora.Linear
    have these paramters:
    r: for all layers expet transformer intrinsic rank
    rs: spatial intrinsic rank
    rt: temporal intrinsic rank
    ...
    def forward(self, x):
        ...
        x = self.temporal_LoraTransformer(x)
