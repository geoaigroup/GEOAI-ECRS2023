...
class FBFFeedForward(nn.Module):
...
class FBFAttention(nn.Module):
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.):
        ...
        linearqkv = nn.Linear(dim, inner_dim * 3)
        linearqkv.bias.requires_grad = False
        self.to_qkv=linearqkv

        linear3 = nn.Linear(inner_dim, dim)
        linear3.bias.requires_grad = True
...
class FBFTransformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0.):
        ...
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                FBFPreNorm(dim, FBFAttention(dim, heads=heads, dim_head=dim_head, dropout=dropout)),
                FBFPreNorm(dim, FBFFeedForward(dim, mlp_dim, dropout=dropout))
            ]))
...
class FBFTSViT(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        self.temporal_transformer = FBFTransformer(self.dim, self.temporal_depth, self.heads, self.dim_head,
                                                   self.dim * self.scale_dim, self.dropout)
        self.space_pos_embedding = nn.Parameter(torch.randn(1, num_patches, self.dim))
        self.space_transformer = FBFTransformer(self.dim, self.spatial_depth, self.heads, self.dim_head,
                                                self.dim * self.scale_dim, self.dropout)
...
class PBFFeedForward(nn.Module):
    def __init__(self, dim, hidden_dim, dropout=0.):
        super().__init__()
        linear1 = nn.Linear(dim, hidden_dim)
        linear1.bias.requires_grad = False
...
class PBFTransformer(nn.Module):
    def __init__(self, dim, depth, heads, dim_head, mlp_dim, dropout=0.):
        super().__init__()
        self.layers = nn.ModuleList([])
        self.norm = nn.LayerNorm(dim)
        for _ in range(depth):
            self.layers.append(nn.ModuleList([
                PBFPreNorm(dim, PBFAttention(dim, heads=heads, dim_head=dim_head, dropout=dropout)),
                PBFPreNorm(dim, PBFFeedForward(dim, mlp_dim, dropout=dropout))
            ]))
...
class PBFTSViT(nn.Module):
    ...
    def __init__(self, model_config):
        ...
        self.to_temporal_embedding_input = nn.Linear(366, self.dim)
        self.temporal_token = nn.Parameter(torch.randn(1, self.num_classes, self.dim))
        self.temporal_transformer = PBFTransformer(self.dim, self.temporal_depth, self.heads, self.dim_head,
                                                   self.dim * self.scale_dim, self.dropout)
        self.space_pos_embedding = nn.Parameter(torch.randn(1, num_patches, self.dim))
        self.space_transformer = PBFTransformer(self.dim, self.spatial_depth, self.heads, self.dim_head,
                                                self.dim * self.scale_dim, self.dropout)
        self.dropout = nn.Dropout(self.emb_dropout)
        linear4 = nn.LayerNorm(self.dim)
        linear5 = nn.Linear(self.dim, self.patch_size ** 2)
        self.mlp_head = nn.Sequential(
            linear4,
            linear5
        )
